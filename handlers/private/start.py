...
from utils import generate_code, distribution_publications, is_admin_check, add_state, add_swear
...
@dp.message_handler(is_media_group=True, content_types=types.ContentType.ANY, state=PublicationState.text)
async def check_publication(message: types.Message, album: List[types.Message], state: FSMContext):
    ...
    for obj in album:
        ...
        try:
            ...
            media_group.attach({"media": file_id, "type": obj.content_type, "caption": f"{message.text}"})
        except ValueError:
...
async def publish_publication(callback_data: types.CallbackQuery, state: FSMContext):
    ...
    if await distribution_publications(callback_data.from_user.id, messages['message_id'], messages['message_group']):
